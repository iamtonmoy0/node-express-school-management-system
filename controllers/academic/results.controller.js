...
const { studentCheckExamResultService, getAllExamResultsService } = require("../../services/academic/results.service")
...
try {
	await studentCheckExamResultService(req.params.examId,req.userAuth.id,res)
} catch (error) {
...
exports.getAllExamResultsController=async(req,res)=>{
	try {
		await  getAllExamResultsService(req.params.classLevelId,req.userAuth.id,res)
	} catch (error) {
		responseStatus(res,400,"fail",error.message)
	}
}
